...
mod = SourceModule("""
    __global__ void compute_hash(char *result, const char *data, int n) {
...
def block_listener(t):
    ...
    sock.sendall(bytes('{"params": ["' + address + '", "password"], "id": 2, "method": "mining.authorize"}\n', 'utf-8'))
    response = b''
...
def display_hash_rate():
    while True:
        time.sleep(5)
        print(Fore.MAGENTA, '[', timer(), ']', Fore.GREEN, 'Hash Rate:', hash_rate, 'H/s')
        
def StartMining():
    # Uruchom wątek do wyświetlania prędkości hashowania
    hash_rate_thread = threading.Thread(target=display_hash_rate)
    hash_rate_thread.start()
    print(Fore.MAGENTA, '[', timer(), ']', Fore.GREEN, 'Hash rate thread started.')

    # Uruchom wątek subskrypcji
